...
export const AiCameraDotsCanvas: React.FC<CameraDotsProps> = ({
  ...
  useEffect(() => {
    ...
    const initParticles = (w: number, h: number) => {
      const safeW = w > 0 ? w : 640;
      const safeH = h > 0 ? h : 480;
    ...
    // Pre-initialize particles immediately so roomParticles is NEVER empty
    initParticles(canvas.clientWidth || 640, canvas.clientHeight || 480);
    ...
    const render = () => {
      ...
      try {
        ...
        if (cw > 0 && ch > 0) {
          ...
          // Guarantee particles are ready
          if (roomParticles.length === 0) {
            initParticles(cw, ch);
          }
          ...
          if (Array.isArray(landmarks) && landmarks.length > 0 && vid && vid.videoWidth > 0 && vid.videoHeight > 0) {
            ...
            if (lm1 && typeof lm1.x === 'number' && typeof lm1.y === 'number') {
              hasFace = true;
              faceX = cw - toX(lm1.x);
              faceY = toY(lm1.y);
            }
            ...
            if (
              lm454 && lm234 && lm10 && lm152 &&
              typeof lm454.x === 'number' && typeof lm234.x === 'number' &&
              typeof lm10.y === 'number' && typeof lm152.y === 'number'
            ) {
              const faceW = Math.abs(toX(lm454.x) - toX(lm234.x));
              const faceH = Math.abs(toY(lm152.y) - toY(lm10.y));
          ...
          for (let i = 0; i < roomParticles.length; i++) {
            ...
            if (!p1 || typeof p1.x !== 'number') continue;
            for (let j = i + 1; j < roomParticles.length; j++) {
              const p2 = roomParticles[j];
              if (!p2 || typeof p2.x !== 'number') continue;
              const dx = p1.x - p2.x;
              const dy = p1.y - p2.y;
          ...
          for (let i = 0; i < WRAP_NODE_COUNT; i++) {
            ...
            const isLower = Math.sin(theta) > 0;
            const radMultiplier = isLower
              ? 1.45 + 0.35 * Math.sin(theta)
              : 1.22 + 0.12 * Math.cos(theta * 2);
          ...
          if (wrapPoints.length >= 2) {
            ...
            for (let i = 0; i < wrapPoints.length; i++) {
              ...
              // Radial connection to nearest background room nodes (guarded against empty roomParticles)
              if (roomParticles.length > 0 && i % 2 === 0) {
                const nearestRoomNode = roomParticles[i % roomParticles.length];
                if (nearestRoomNode && typeof nearestRoomNode.x === 'number' && typeof nearestRoomNode.y === 'number') {
        ...
        console.warn('AiCameraDotsCanvas: caught render exception safely:', err);
      }
...
export const AiPageDotsBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Pre-initialize particles immediately
    init(canvas.clientWidth || window.innerWidth || 1280, canvas.clientHeight || window.innerHeight || 800);
    ...
    const render = () => {
      ...
      try {
        ...
        console.warn('AiPageDotsBackground: caught render exception safely:', err);
      }
